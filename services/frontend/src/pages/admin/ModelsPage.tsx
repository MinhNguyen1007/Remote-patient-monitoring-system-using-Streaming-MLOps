...
const GATE_LABEL: Record<string, string> = { PROMOTED: 'promote', REJECTED: 'từ chối' };

/** Kích hoạt retrain (UC10): nhận dag_run_id rồi hỏi trạng thái định kỳ, không giữ request chờ DAG. */
function RetrainControl({ onFinished }: { onFinished: () => void }) {
  const [run, setRun] = useState<RetrainStatus | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [busy, setBusy] = useState(false);

  useEffect(() => {
    if (run && TERMINAL.has(run.state)) onFinished();
...
export function ModelsPage() {
  ...
  // Admin nhận sự kiện drift/retrain qua WebSocket (kể cả retrain tự động do drift): tải lại số liệu
  const reloadModels = models.reload;
  const reloadDrift = drift.reload;
  const reloadAll = useCallback(() => {
    reloadModels();
    reloadDrift();
  }, [reloadModels, reloadDrift]);
  useServerEvents((event) => {
    if (event.type === 'drift_report') reloadDrift();
    if (event.type === 'retrain_completed') reloadAll();
  });
