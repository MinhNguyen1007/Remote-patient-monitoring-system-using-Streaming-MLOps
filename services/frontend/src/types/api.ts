// Kiểu dữ liệu khớp schema Pydantic của backend (services/backend/app/schemas/*) và sự kiện Kafka
// (services/streaming: rpm_streaming/consumer/main.py; ml: rpm_ml/drift/detect.py, rpm_ml/pipelines/retrain.py).
...
export interface ModelVersion {
  ...
  /** Lý do bị quality gate từ chối, nối bằng "; " (null khi đạt). */
...
export interface DriftReport {
  ...
  max_psi: number | null;
  /** PSI/KS từng đặc trưng; `threshold` = ngưỡng hiệu chỉnh (≥ 0,25), `drifted` khi PSI ≥ threshold. */
  feature_stats: Record<string, DriftFeatureStat> | null;
...
export interface DriftFeatureStat {
  ...
  threshold?: number;
  drifted?: boolean;
  level?: 'negligible' | 'moderate' | 'significant';
}
...
export interface RetrainStatus {
  ...
  /** Version do lần chạy này tạo, kèm kết quả quality gate. */
...
export type ServerEvent =
  ...
  | { type: 'drift_report'; data: DriftReportEvent }
  | { type: 'retrain_completed'; data: RetrainCompletedEvent };

/** Sự kiện `drift_report` (mlops-events, chỉ Admin nhận). */
export interface DriftReportEvent {
  ...
  triggered_retrain: boolean;
  retrain_dag_run_id: string | null;
  retrain_blocked_reason: string | null;
  notify_admin: boolean;
}

/** Sự kiện `retrain_completed` (mlops-events, chỉ Admin nhận). */
export interface RetrainCompletedEvent {
  dag_run_id: string;
  trigger: 'MANUAL' | 'DRIFT';
  results: { model_name: string; status: 'PROMOTED' | 'REJECTED' | 'FAILED'; version?: string; gate_reasons?: string[] }[];
}
