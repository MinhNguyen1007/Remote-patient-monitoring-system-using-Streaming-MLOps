...
const THRESHOLDS = [
  ...
  { value: 0.25, label: '0,25 · sàn của ngưỡng drift', color: CHART.risk.CRITICAL },
];

/**
 * Max PSI mỗi lần kiểm tra drift (mục 2.9.4). Điểm đỏ = drift: có đặc trưng vượt ngưỡng hiệu chỉnh riêng của nó
 * (luôn ≥ 0,25), nên max PSI trên 0,25 chưa chắc là drift. Tooltip nêu đặc trưng lệch và việc kích hoạt retrain.
 */
      ...
      {hovered && hover !== null && (
        <div className="pointer-events-none absolute top-4 flex w-[220px] flex-col gap-1 border border-border bg-popover px-3 py-2.5 text-[12.5px]" style={{ left: x(hover) > width / 2 ? x(hover) - 236 : x(hover) + 14 }}>
          ...
          {Object.entries(hovered.feature_stats ?? {})
            ...
            .map(([feature, stat]) => (
              <span key={feature} className="mono text-[11.5px]">{feature} {vn(stat.psi, 2)} ≥ {vn(stat.threshold, 2)}</span>
            ))}
        </div>
      )}
    </div>
