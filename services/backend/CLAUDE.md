...
app/
  ...
  events/         dispatcher.py (đẩy WS tới người được phân công + email; sự kiện MLOps tới Admin), listener.py
                  (Kafka predictions-stream, alerts-stream, mlops-events → dispatcher, thread nền)
  services/       notification (email cảnh báo + email drift, notification_logs), ws_manager, airflow_client, mlflow_client
  ...
  alembic/        Migration (0001: toàn bộ schema + hypertable; 0002: notification_logs.drift_report_id, model_versions.gate_reasons)
tests/            pytest trên DB `rpm_test` thật (TimescaleDB), 34 test
...
| `GET /admin/models`, `GET /admin/drift-reports` | ADMIN | UC09; version kèm `gate_reasons`; báo cáo drift kèm `feature_stats` (psi, threshold, drifted) |
| `POST /admin/models/retrain` → 202, `GET /admin/models/retrain/{dag_run_id}` | ADMIN | UC10; trạng thái kèm `results` (model_versions cùng `dag_run_id`); Airflow lỗi → 502 |
| `WS /ws?token=<JWT>` | mọi role | Sự kiện `{type, data}`: `connected`, `prediction`, `alert`, `alert_update`; Admin nhận `drift_report`, `retrain_completed`; token sai → đóng 1008; gửi `ping` nhận `pong` |
...
- Mọi model SQLAlchemy trong `db/models.py` phải khớp đúng entity/attribute trong `docs/design/02_5_class.md` và `02_7_erd.md` — nếu cần thêm cột, cập nhật ERD trước.
  ...
  - Consumer (`../streaming/`) ghi trực tiếp bằng SQL vào patients, vital_records, predictions, alerts, model_versions và seed alert_settings — đổi tên cột phải sửa cả `../streaming/src/rpm_streaming/storage/repository.py` và `../../ml/src/rpm_ml/storage/db.py`.
...
- Sự kiện MLOps (topic `mlops-events`, do DAG Airflow publish): chỉ tới Admin đang hoạt động. `drift_report` có `notify_admin` → email (dedupe theo `notification_logs.drift_report_id`); `retrain_completed` chỉ đẩy WebSocket. Airflow không tự gửi email.
