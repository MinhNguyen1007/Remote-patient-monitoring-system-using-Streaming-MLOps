...
def retrain_status(dag_run_id: str, request: Request, db: Session = Depends(get_db)) -> RetrainStatus:
    """Trạng thái lần chạy DAG + kết quả gate đã ghi vào model_versions (02_4 mục 2.4.3)."""
    try:
        run = airflow(request).get_run(dag_run_id)
    except AirflowError as exc:
        raise HTTPException(status.HTTP_502_BAD_GATEWAY, str(exc)) from None
    results = db.scalars(
        select(ModelVersion).where(ModelVersion.dag_run_id == dag_run_id).order_by(ModelVersion.model_name)
    ).all()
    return RetrainStatus(
        ...
        results=[ModelVersionOut.model_validate(version) for version in results],
    )
