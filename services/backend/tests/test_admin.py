...
def test_retrain_status_includes_gate_results_of_that_run(client, make, db):
    from app.db.models import GateStatusEnum, ModelVersion, TriggerEnum

    for name, status, reasons in (
        ("risk_classifier", GateStatusEnum.REJECTED, "Recall CRITICAL 0.740 < ngưỡng 0.75"),
        ("anomaly_detector", GateStatusEnum.PROMOTED, None),
    ):
        db.add(ModelVersion(id=uuid.uuid4(), model_name=name, mlflow_version="9", mlflow_run_id="r", gate_status=status,
                            gate_reasons=reasons, is_champion=status == GateStatusEnum.PROMOTED,
                            trigger=TriggerEnum.MANUAL, dag_run_id="manual__7"))
    db.commit()
    client.app.state.airflow = airflow_mock(lambda request: httpx.Response(200, json={
        "dag_run_id": "manual__7", "state": "success", "start_date": None, "end_date": None, "conf": {}}))
    try:
        body = client.get("/admin/models/retrain/manual__7", headers=make.headers(make.user("ADMIN"))).json()
        assert body["state"] == "success"
        assert [(r["model_name"], r["gate_status"]) for r in body["results"]] == [
            ("anomaly_detector", "PROMOTED"), ("risk_classifier", "REJECTED")]
        assert body["results"][1]["gate_reasons"].startswith("Recall CRITICAL")
    finally:
        client.app.state.airflow = None
