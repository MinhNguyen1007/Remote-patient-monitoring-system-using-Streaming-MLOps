...
một lần gọi `predict`. File này được đính kèm model qua `code_paths` (`serving_code_paths`); consumer chỉ cần cài
`rpm_common` và tensorflow, không cần cài `rpm_ml`.
"""
...
def serving_code_paths(workdir: Path) -> list[str]:
    """Khung package tối thiểu `rpm_ml/models/anomaly.py` để chụp kèm model qua `code_paths`.

    Wrapper được pickle theo tên module `rpm_ml.models.anomaly`; MLflow thêm thư mục code của model vào `sys.path` khi
    nạp, nên consumer (không cài `rpm_ml`) vẫn import được đúng module này — và chỉ module này, không kéo theo phần
    huấn luyện. Version cũ (≤ v5) mang file `anomaly_model.py` riêng của chúng nên vẫn nạp được như trước.
    """
    package = Path(workdir) / "rpm_ml"
    (package / "models").mkdir(parents=True, exist_ok=True)
    (package / "__init__.py").write_text("", encoding="utf-8")
    (package / "models" / "__init__.py").write_text("", encoding="utf-8")
    shutil.copyfile(__file__, package / "models" / "anomaly.py")
    return [str(package)]
