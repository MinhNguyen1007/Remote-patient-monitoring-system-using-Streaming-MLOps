...
| Smoke test hạ tầng (chạy mỗi khi sửa `docker-compose.yml` hoặc Dockerfile trong `infra/`) | • Postgres có đủ 4 database và bật `timescaledb`.<br>• Từ host produce/consume qua `localhost:29092`.<br>• Log 1 model thử từ host lên MLflow rồi tải lại **từ bên trong container khác**.<br>• Gọi Airflow REST API bằng basic auth trả 200 và thấy đủ 2 DAG.<br>• Prometheus scrape được `backend` (không chỉ scrape chính nó).<br>• Grafana nạp được dashboard trong provisioning và giải được datasource theo uid. | `infra/smoke_test.py` (thành phần chưa chạy thì bỏ qua kèm lời nhắc, không báo lỗi) | **6/6 đạt** (2026-09-12) |
