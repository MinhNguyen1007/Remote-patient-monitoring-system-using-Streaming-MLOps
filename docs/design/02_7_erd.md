# 2.7. Biểu đồ mối quan hệ giữa các dữ liệu (ERD)

```mermaid
erDiagram
    USERS ||--o{ PATIENT_ASSIGNMENTS : "được phân công"
    PATIENTS ||--o{ PATIENT_ASSIGNMENTS : "có người phụ trách"
    PATIENTS ||--o{ VITAL_RECORDS : "có"
    VITAL_RECORDS ||--o{ PREDICTIONS : "sinh ra"
    PREDICTIONS ||--o{ ALERTS : "có thể tạo (tối đa 1 mỗi loại)"
    PATIENTS ||--o{ ALERTS : "thuộc về"
    USERS |o--o{ ALERTS : "xác nhận/xử lý"
    ALERTS |o--o{ NOTIFICATION_LOGS : "ghi log gửi"
    DRIFT_REPORTS |o--o{ NOTIFICATION_LOGS : "ghi log thông báo drift"
    USERS ||--o{ NOTIFICATION_LOGS : "người nhận"
    MODEL_VERSIONS ||--o{ PREDICTIONS : "model rủi ro"
    MODEL_VERSIONS |o--o{ PREDICTIONS : "model bất thường"
    MODEL_VERSIONS ||--o{ DRIFT_REPORTS : "làm mốc so sánh"
    DRIFT_REPORTS |o--o{ MODEL_VERSIONS : "kích hoạt huấn luyện"
    USERS |o--o{ ALERT_SETTINGS : "cập nhật"

    USERS {
        uuid id PK
        string full_name
        string email UK
        string password_hash
        string role "ADMIN | DOCTOR | NURSE"
        bool is_active
        timestamptz created_at
    }

    PATIENTS {
        uuid id PK
        string display_name
        string gender
        int age
        int mimic_subject_id UK
        int mimic_icustay_id UK
        timestamptz admitted_at
    }

    PATIENT_ASSIGNMENTS {
        uuid id PK
        uuid patient_id FK
        uuid user_id FK "role DOCTOR hoặc NURSE"
        uuid assigned_by FK "Admin"
        timestamptz assigned_at
    }

    VITAL_RECORDS {
        uuid patient_id PK, FK
        timestamptz recorded_at PK "cột phân vùng hypertable"
        int hour_index
        float heart_rate "giá trị đo, chưa điền"
        float spo2
        float respiratory_rate
        float systolic_bp
        float diastolic_bp
        float temperature
    }

    PREDICTIONS {
        uuid id PK
        timestamptz recorded_at PK "cột phân vùng hypertable"
        uuid patient_id FK "cùng recorded_at tham chiếu VITAL_RECORDS"
        timestamptz predicted_at
        int news2_score
        string risk_level "NORMAL | WARNING | CRITICAL"
        float risk_score "P(CRITICAL)"
        float anomaly_score "0-1, rỗng khi chưa đủ cửa sổ"
        bool is_anomaly
        uuid risk_model_version_id FK
        uuid anomaly_model_version_id FK
    }

    ALERTS {
        uuid id PK
        uuid patient_id FK
        uuid prediction_id "tham chiếu logic tới PREDICTIONS"
        timestamptz prediction_recorded_at
        string alert_type "RISK | ANOMALY"
        string status "OPEN | ACKNOWLEDGED | RESOLVED"
        timestamptz created_at
        uuid acknowledged_by FK
        timestamptz acknowledged_at
        uuid resolved_by FK
        timestamptz resolved_at
        string resolution_note
    }

    NOTIFICATION_LOGS {
        uuid id PK
        uuid alert_id FK "rỗng khi là thông báo drift"
        uuid drift_report_id FK "rỗng khi là cảnh báo"
        uuid recipient_user_id FK
        string channel "EMAIL"
        timestamptz sent_at
        string status "SENT | FAILED"
        string error_message
    }

    ALERT_SETTINGS {
        uuid id PK
        float risk_critical_threshold "rỗng = dùng ngưỡng của model champion"
        float anomaly_threshold
        int cooldown_hours
        uuid updated_by FK
        timestamptz updated_at
    }

    MODEL_VERSIONS {
        uuid id PK
        string model_name "risk_classifier | anomaly_detector"
        string mlflow_version "UK cùng model_name"
        string mlflow_run_id
        string gate_status "PROMOTED | REJECTED"
        string gate_reasons "lý do gate từ chối"
        bool is_champion
        string trigger "INITIAL | DRIFT | MANUAL"
        uuid drift_report_id FK
        string dag_run_id
        json metrics
        timestamptz trained_at
    }

    DRIFT_REPORTS {
        uuid id PK
        timestamptz run_at
        timestamptz window_start
        timestamptz window_end
        int n_records
        uuid reference_model_version_id FK
        float max_psi
        json feature_stats "PSI và KS từng đặc trưng"
        bool drift_detected
        bool triggered_retrain
        string dag_run_id
    }
```

**Diễn giải cardinality và ràng buộc quan trọng:**
- `PATIENTS (1) — (0..*) PATIENT_ASSIGNMENTS (0..*) — (1) USERS`: quan hệ nhiều–nhiều giữa bệnh nhân và bác sĩ/điều dưỡng. Ràng buộc duy nhất `(patient_id, user_id)`. Role của `user_id` phải là `DOCTOR` hoặc `NURSE`, kiểm tra ở tầng ứng dụng.
- `PATIENTS (1) — (0..*) VITAL_RECORDS`: dữ liệu time-series. Khóa chính là cặp tự nhiên `(patient_id, recorded_at)`, mỗi bệnh nhân có tối đa 1 bản ghi mỗi giờ dữ liệu.
  - 6 cột vitals lưu **giá trị đo như producer gửi, chưa forward-fill**. Giờ không đo thì để trống. Consumer cần giá trị gốc để dựng lại đặc trưng giống hệt lúc huấn luyện khi khởi động lại. Giá trị đã điền được tính lại khi cần và có trong message `predictions-stream` (`vitals_filled`).
- `VITAL_RECORDS (1) — (0..*) PREDICTIONS`:
  - Luồng chính tạo đúng 1 prediction cho mỗi bản ghi.
  - Tái dự đoán bằng model mới sẽ thêm dòng mới, không sửa dòng cũ.
  - Liên kết qua cặp `(patient_id, recorded_at)`.
- `PREDICTIONS (1) — (0..2) ALERTS`: tối đa 1 cảnh báo `RISK` và 1 cảnh báo `ANOMALY` cho mỗi prediction.
- `USERS (0..1) — (0..*) ALERTS`: `acknowledged_by`/`resolved_by` để trống khi cảnh báo chưa được xử lý.
- `MODEL_VERSIONS (1) — (0..*) PREDICTIONS` (model rủi ro, bắt buộc) và `(0..1) — (0..*)` (model bất thường, để trống khi chưa đủ cửa sổ 12 giờ).
- `MODEL_VERSIONS`: ràng buộc duy nhất `(model_name, mlflow_version)`.
  - Consumer tự đồng bộ version champion từ MLflow vào bảng này khi nạp model (`gate_status = PROMOTED`, `is_champion = true`, bỏ cờ champion ở version cũ).
  - DAG `retrain_pipeline` (Giai đoạn G) ghi thêm các challenger, kể cả version bị từ chối.
- `MODEL_VERSIONS (1) — (0..*) DRIFT_REPORTS`: mỗi lần kiểm tra drift so với phân phối huấn luyện của đúng 1 version champion.
- `DRIFT_REPORTS (0..1) — (0..*) MODEL_VERSIONS`: các challenger được huấn luyện do một lần drift kích hoạt trỏ ngược về báo cáo đó. Retrain thủ công hoặc lần huấn luyện đầu tiên thì để trống.
  - `gate_reasons` lưu lý do bị quality gate từ chối (vd "Recall CRITICAL 0,740 < champion 0,790"), để trống khi đạt.
- `NOTIFICATION_LOGS`: mỗi dòng gắn với **đúng một** trong hai đối tượng (CHECK constraint): cảnh báo (`alert_id`, email tới bác sĩ/điều dưỡng) hoặc báo cáo drift (`drift_report_id`, email tới Admin — Giai đoạn G, migration `0002`). Đã có `SENT` cho cặp (đối tượng, người nhận) thì không gửi lại.

**Lưu ý hiện thực với TimescaleDB:**
- `vital_records` và `predictions` là hypertable, phân vùng theo `recorded_at`. TimescaleDB bắt buộc mọi khóa chính/unique của hypertable phải chứa cột phân vùng, nên khóa chính là `(patient_id, recorded_at)` và `(id, recorded_at)`, không phải `id` đơn lẻ.
- Khóa ngoại **từ** hypertable **tới** bảng thường (`patients`, `model_versions`) được ràng buộc ở DB như bình thường. Khóa ngoại **trỏ vào** hypertable (`predictions → vital_records`, `alerts → predictions`) bị giới hạn tùy phiên bản TimescaleDB, nên được giữ là **tham chiếu logic**. Tính toàn vẹn đảm bảo ở tầng ứng dụng: consumer ghi `vital_record`, `prediction` và `alert` trong cùng một transaction.
- Các bảng còn lại dùng bảng PostgreSQL thông thường.
